...
const results = [];
const errors = [];
const OUTPUT_DIR = `output-pattern-insights-${Date.now()}`;
...
请基于上述标注和原始图表，生成【模式特征】和【核心洞察】两个部分。`;
    ...
    console.error(`❌ [${index + 1}/${total}] 失败: ${image_id}`);
...
async function main() {
  ...
  // 从数据库获取已标注的数据
  console.log('📊 正在从数据库读取已标注数据...');
  const allAnnotations = await getAnnotatedData();
  console.log(`✅ 数据库中共有 ${allAnnotations.length} 条已标注数据`);

  // 只处理后800条
  const annotations = allAnnotations.slice(-800);
  console.log(`📌 本次将处理后 ${annotations.length} 条数据\n`);

  if (annotations.length === 0) {
    console.log('❌ 没有找到已标注的数据，程序退出。');
    return;
  }
