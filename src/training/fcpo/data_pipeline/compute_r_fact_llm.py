"""
compute_r_fact_llm.py — LLM 版 r_fact 计算（独立评分版）

对 chosen 和 rejected 分别独立调用 API，使用完全相同的 prompt / 模型 / 参数。
模型不知道哪段文本是 chosen、哪段是 rejected，消除 label leakage。

Tier 3（real_no_data）跳过 API 调用，delta_r_fact = 0。

使用方式:
  python compute_r_fact_llm.py --test 5            # 测试 5 条
  python compute_r_fact_llm.py --max-workers 8     # 全量 8 并发（每对 2 次独立调用）
  python compute_r_fact_llm.py --resume            # 断点续传
"""
...
DEFAULT_OUTPUT_PATH = V3_DATA_DIR / "r_fact_llm.json"
DEFAULT_CHECKPOINT_PATH = V3_DATA_DIR / ".r_fact_llm_checkpoint.json"
...
**Scoring scale (1-10):**
- 10: All verifiable factual claims are accurate
- 8-9: 1-2 minor errors (small rounding, slight date imprecision)
- 6-7: Several factual errors or 1-2 significant errors
- 4-5: Many factual errors; source data is misrepresented
- 1-3: Pervasive factual errors; directly contradicts source data
...
def process_single(client: FactJudgeClient, idx: int, source_entry: dict, dpo_entry: dict) -> dict:
    ...
    r_fact_chosen = round(chosen_result["score"] / 10.0, 4)
    r_fact_rejected = round(rejected_result["score"] / 10.0, 4)
