#!/bin/bash
# LR/Epoch 消融实验 (训练 + 推理)
# LF-3: beta=0.1, lr=5e-6,  rank=256, 1 epoch, 1700 条
# LF-4: beta=0.1, lr=2e-5,  rank=256, 1 epoch, 1700 条
# LF-6: beta=0.1, lr=1e-5,  rank=256, 2 epoch, 1700 条
#
# 数据: dpo_train_1700_swift.jsonl (ms-swift 格式 → LF sharegpt JSON)
# 训练: 串行 3 个实验 ~15min 各
...
echo "=========================================="
echo " LR/Epoch Ablation (LF-3, LF-4, LF-6)"
echo " $(date)"
echo "=========================================="
...
$OSSUTIL cp oss://qwen3vl-dpo-training/data/dpo_train_1700_swift.jsonl $DATA_ROOT/dpo/data/dpo_train_1700_swift.jsonl $OSS &
wait
...
# === 3. Convert data to LF sharegpt format ===
echo "=== [3/5] Convert data ==="
python3 << 'PYEOF'
import json, os

IMG_DIR = '$DATA_ROOT/dpo/data/images'
SRC = '$DATA_ROOT/dpo/data/dpo_train_1700_swift.jsonl'
...
for line in open(SRC):
    ...
    # ms-swift 格式: messages 包含 user/assistant（chosen），rejected_response 单独字段
    user_msgs = []
    chosen_text = None
    for m in msgs:
...
out_path = '$DATA_ROOT/dpo/data/dpo_train_1700_lf.json'
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(records, f, ensure_ascii=False)

print(f'  dpo_train_1700: {len(records)} records (skipped {skipped})')
PYEOF
...
{
  "dpo_train_1700": {
    "file_name": "$DATA_ROOT/dpo/data/dpo_train_1700_lf.json",
    "formatting": "sharegpt",
    "ranking": true,
    "columns": {
...
# === 4. Train ===
echo "=== [4/5] Train (3 ablation experiments) ==="
TRAIN_OK=()

run_exp() {
    local NAME=$1 LR=$2 EPOCHS=$3
    echo ""
    echo "========== Training $NAME (lr=$LR, epochs=$EPOCHS) =========="
    echo "  Start: $(date)"
...
run_exp "LF-3" "5.0e-6" "1.0"
run_exp "LF-4" "2.0e-5" "1.0"
run_exp "LF-6" "1.0e-5" "2.0"
