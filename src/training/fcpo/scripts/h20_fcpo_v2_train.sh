#!/bin/bash
# V3 FCPO-v2 Training Pipeline: FCPO-γ1, FCPO-γ2, FCPO-γ4
# 基于 V2 LF-full 验证通过的环境 + FCPO patch
#
# 训练 → Merge → Inference → Upload
# 数据: fcpo_v2_gamma{1,2,4}.jsonl (含 margin 字段)
...
echo "=========================================="
echo " V3 FCPO-v2 Training Pipeline"
echo " $(date)"
echo "=========================================="
...
wait
echo "FCPO data: $(ls -1 $DATA_ROOT/dpo/data/fcpo_v2_gamma*.jsonl 2>/dev/null | wc -l) files"
...
for g in [1, 2, 4]:
    convert_fcpo(
        f'$DATA_ROOT/dpo/data/fcpo_v2_gamma{g}.jsonl',
        f'$DATA_ROOT/dpo/data/fcpo_v2_gamma{g}_lf.json',
        f'gamma{g}'
    )
PYEOF
...
{
  "fcpo_v2_gamma1": {
    "file_name": "$DATA_ROOT/dpo/data/fcpo_v2_gamma1_lf.json",
    "formatting": "sharegpt",
    "ranking": true,
    "columns": {"messages": "conversations", "chosen": "chosen", "rejected": "rejected", "images": "images"}
  },
  "fcpo_v2_gamma2": {
    "file_name": "$DATA_ROOT/dpo/data/fcpo_v2_gamma2_lf.json",
    "formatting": "sharegpt",
    "ranking": true,
    "columns": {"messages": "conversations", "chosen": "chosen", "rejected": "rejected", "images": "images"}
  },
  "fcpo_v2_gamma4": {
    "file_name": "$DATA_ROOT/dpo/data/fcpo_v2_gamma4_lf.json",
    "formatting": "sharegpt",
    "ranking": true,
    "columns": {"messages": "conversations", "chosen": "chosen", "rejected": "rejected", "images": "images"}
  }
